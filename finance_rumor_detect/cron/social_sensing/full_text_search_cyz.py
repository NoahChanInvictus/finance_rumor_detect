...
def get_economics_mids(mid_topic_value):
    ...
    for k,v in mid_topic_value.iteritems():
        if v == 7:
            items.append(k)
    return items
def save_results(items,ts,name):
    #print items
    result = ''
    for item in items:
        print item[0][1]
        result+=item[0][1]+' '
        result+=item[0][3]+'\n'
    with open('./temp1/'+name+str(ts)+'.txt','w') as tf:
...
if __name__ == '__main__':
    ...
    size = 100
    type_value = []
    total_rank = get_origin_weibo_detail(ts,size,'total')
    comment_rank = get_origin_weibo_detail(ts,size,'comment')
    retweet_rank = get_origin_weibo_detail(ts,size,'retweet')
    save_results(total_rank,ts,'total')
    save_results(retweet_rank,ts,'retweet')
    save_results(comment_rank,ts,'comment')
